...
def accuracy_precision_table(filtered_table, label_folder, class_masks_folder, tabrowcl_filename, mann_whitney_table):
    ''' 
    This function creates a table with accuracy and precision of every image in the folder
    '''
    #import statements
    from skimage.io import imread, imsave
    from pyclesperanto_prototype import replace_intensities
    from skimage.measure import label, regionprops
    import numpy as np
    import pandas as pd
    import apoc
    ...
    slice_list = make_slices(number_elements)
    single_image_accuracy_list = []
    single_image_precision_list = []
    for slc in slice_list:
        # get annotation for every single image
        annotation_per_image = table['annotated_class'][slc]
        annotation_mask = annotation_per_image > 0
    
        # get prediction for every single image
        prediction_per_image = table['predicted_class'] [slc]
    
        #accuracy 
        single_image_accuracy_list.append(accuracy_score(annotation_per_image[annotation_mask], prediction_per_image[annotation_mask]))
    
        #precision
        single_image_precision_list.append(precision_score(annotation_per_image[annotation_mask], prediction_per_image[annotation_mask]))
    df = pd.DataFrame(list(zip(single_image_accuracy_list, single_image_precision_list)), columns = ['accuracy','precision'])
    df.to_csv(mann_whitney_table)
